...
import { getConfluenceVotes } from "../src/strategy/confluence-votes.js";
...
export function runBacktest(opts) {
  const {
    ...
    minStartIndex = 100,
    requireVotes = 2,     // 0 = disable vote gate entirely (behaves exactly like engine.js)
    voteParams = {},
  } = opts;
  ...
  let votesSeen = 0, votesVetoed = 0; // how many Stage1-3 signals fired vs. how many the vote gate blocked
  ...
  for (let i = minStartIndex; i < m15.length; i++) {
    ...
    try {
      ...
      // Compute votes HERE, still inside the try block, while tf's
      // arrays still have the forming-candle placeholder appended —
      // getConfluenceVotes strips the last element internally the
      // same way collectSignals's own helpers do. Computing this
      // after the finally block pops the placeholder would silently
      // drop a real closed candle instead, which would be wrong.
      if (requireVotes > 0 && (signalResult.signal === SIG_BUY || signalResult.signal === SIG_SELL)) {
        const direction = signalResult.signal === SIG_BUY ? "bullish" : "bearish";
        voteResult = getConfluenceVotes(tf, direction, { ...voteParams, requireVotes });
      }
    } finally {
      growingD1.pop();
      growingH1.pop();
    ...
    // ── NEW: confluence vote gate ──
    // The Stage 1-3 entry decision (signalResult.signal) is unchanged —
    // this only decides whether to ACT on a signal that already fired.
    if (voteResult) {
      votesSeen++;
      if (!voteResult.approved) {
        votesVetoed++;
        return; // signal fired, but confluence votes didn't clear the bar — skip this trade
      }
    }
  ...
  return summarize({ symbol, trades, equityCurve, startEquity, finalEquity: equity, votesSeen, votesVetoed });
}

function summarize({ symbol, trades, equityCurve, startEquity, finalEquity, votesSeen = 0, votesVetoed = 0 }) {
